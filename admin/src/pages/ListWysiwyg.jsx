...
const ListWysiwyg = ({ token }) => {
  ...
  // ✅ Filter produk berdasarkan `latinName`
  const filteredProducts = products.filter(product =>
    product.latinName.toLowerCase().includes(searchQuery.toLowerCase())
  );

  // ✅ Filter hasil yang sudah dikelompokkan
  const filteredGroupedProducts = Object.entries(groupedProducts).reduce((acc, [latinName, products]) => {
    if (latinName.toLowerCase().includes(searchQuery.toLowerCase())) {
      acc[latinName] = products;
    }
    return acc;
  }, {});
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get(`${backendUrl}/api/product/listWysiwyg`);
      if (response.data.success) {
        const productList = response.data.wysiwygProducts;
        setProducts(response.data.wysiwygProducts);
        setTotalProducts(response.data.wysiwygProducts.length); // ✅ Simpan total produk
  ...
  const executeDelete = async () => {
    ...
    try {
      if (deleteType === "single" && deleteTarget) {
        // 🔥 Hapus satu produk
        const response = await axios.post(`${backendUrl}/api/product/remove-wysiwyg`, { id: deleteTarget }, { headers: { token } });
        ...
        const response = await axios.post(`${backendUrl}/api/product/remove-multiple-wysiwyg`, { ids: selectedProducts }, { headers: { token } });
        ...
        // 🔥 Hapus semua produk
        const response = await axios.post(`${backendUrl}/api/product/remove-all-wysiwyg`, {}, { headers: { token } });
  ...
  const updateProduct = async (e) => {
    ...
    try {
      const updatedData = {
        ...
        latinName: editingProduct.latinName,
        code: editingProduct.code,
        latin_name: editingProduct.latin_name,
        commonName: editingProduct.commonName,
        size: editingProduct.size,
        value: editingProduct.value,
        plasticSize: editingProduct.plasticSize,
        heightCm: editingProduct.heightCm,
      ...
      const response = await axios.post(`${backendUrl}/api/product/update-wysiwyg`, updatedData, {
  ...
  const importData = async () => {
    setImporting(true);
    try {
      const response = await axios.get(`${backendUrl}/api/importOfferWysiwyg`);
      if (response.data.success) {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 ">
      <h2 className="text-2xl font-bold text-gray-800 mb-4">WYSIWYG Product List</h2>
      {/* 🔍 Input Pencarian */}
      <div className="mb-4">
        <label className="block font-semibold text-gray-700">Search by Latin Name</label>
        <input
          type="text"
          value={searchQuery}
                ...
                {products.map((product) => (
                  <div key={product._id} className="bg-white shadow-md rounded-lg overflow-hidden p-4 relative">
                    ...
                    <div className="mt-3">
                      ...
                      <p className="text-gray-500 text-sm">{product.latinName}</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center p-4">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
            ...
            <form onSubmit={updateProduct} className="space-y-4">
              ...
              {/* Latin Name */}
              <div>
                <label className="block text-gray-700 font-semibold mb-1">Latin Name</label>
                <input
                  type="text"
                  value={editingProduct.latinName}
                  onChange={(e) => setEditingProduct({ ...editingProduct, latinName: e.target.value })}
              ...
              <div>
                ...
                <input
                  type="text"
                  value={editingProduct.commonName}
                  onChange={(e) => setEditingProduct({ ...editingProduct, commonName: e.target.value })}
                  className="w-full px-3 py-2 border rounded-md"
                  required
                />
              </div>
